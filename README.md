...
[Mission](#part-of-a-broader-local-first-mission) · [How it works](#what-redactguard-does) · [Screens](#product-walkthrough) · [Value](#the-value-it-adds) · [Architecture](#architecture) · [Privacy](#privacy-and-data-lifecycle) · [Run locally](#run-locally)
...
The interface keeps the workflow explicit from the first screen: upload locally, review every model finding, and export only after confirmation.

![RedactGuard product overview](docs/assets/screenshots/redactguard-overview.jpg)

### 1. Choose a profile and load a PDF

Select the profile closest to the document. Each profile focuses detection on the PII categories that matter for that context; the file remains on the device throughout the workflow.

![Healthcare profile selected with a PDF ready to upload](docs/assets/screenshots/redactguard-upload.jpg)

### 2. Review the detected sensitive data
...
### 3. Export the anonymized output
...
> [!NOTE]
> The walkthrough uses a synthetic clinical note created specifically for this README. It contains no real personal or health data, and the displayed findings are illustrative rather than a model benchmark.
